...
public class SWDC_ring extends RegularGraph{
	...
	public static int DIM = 1;			// Dimension is different for different SWDC_rings; HexTor = 3;
	public static int NUM_RAND_CONN = 4;		// Num-rand-ports is different for different SWDC_rings; HexTor = 1;

	public SWDC_ring(int size, int degree){
		super(size,degree);
...
//		System.out.println("ALREADY-ADDED HAS : " + alreadyAdded.size());
	...
	public void PrintGraphforMCF(String filename, int trafficmode, int transportprotocol){
		...
		int svrs = r*2;
//		System.out.println("CY PRINT RING" + svrs);
...
//								objective += "\n";
	...
	private void populateAdjacencyList(int swdeg, int degree, int mode, int ex_size, int nsvrs){
		...
		for (int r = 0; r < noNodes; r++)
			addBidirNeighbor(r, (r + 1) % noNodes);
		...
		for (int i = 0; i < noNodes; i ++) 
			for (int j = 0; j < randPorts; j++) {
				to = randomEdges[i][j];
				if (randomEdges[to][j] != i) System.out.println("MISMATCHED edges");
				if (to < i) addBidirNeighbor(i, to);
			}
	}
/*
	public static void main(String[] args){
		...
		Graph net = new SWDC_ring(numNodes, swports, -1, 1, 0, numsrv);
