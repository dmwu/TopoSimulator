...
public class PartialFlatTreeCluster extends Graph{
	...
	public PartialFlatTreeCluster(int K_, int flat_pod_num){
		super(K_*K_*5/4);
		this.K = K_;
		setParameters(K_);
	...
	public PartialFlatTreeCluster(int K_, int flat_pod_num, double fail_rate){
		super(K_*K_*5/4);
		this.K = K_;
	...
	private void populateAdjacencyList(){
		...
		for (int i = flatPodNum; i < podNum; i++) {
			...
			for (int j = 0; j < aggrSwitchPerPod; j++) {
				int aggrIndex = i * aggrSwitchPerPod + j;
				weightEachNode[getNodeIndex("A"+aggrIndex)] = hostPerRealAggr;
	...
	public int svrToSwitch(int i)	//i is the server index. return the switch index.
	{
		...
		if (i < hostPerPod * flatPodNum) {
			...
			int podIndex = indexInZone / hostPerPod;
			int hostInPod = indexInZone - podIndex * hostPerPod;
			int pairIndex = hostInPod / (K/2);
			int hostInPair = hostInPod - pairIndex * (K/2);
			int index = (podIndex + flatPodNum) * edgeSwitchPerPod + pairIndex;
	...
	/*public static void main (String args[]) {
		PartialFlatTreeCluster myTree = new PartialFlatTreeCluster(6, 2);
