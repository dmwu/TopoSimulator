...
public class FlatTreeClusterUniform extends Graph{
	...
	public FlatTreeClusterUniform(int K_){
		super(K_*K_*5/4);
		this.K = K_;
		setParameters(K_);
	...
	public FlatTreeClusterUniform(int K_, double fail_rate){
		super(K_*K_*5/4);
		this.K = K_;
	...
	public void setParameters (int k) {
		...
		hostPerRealAggr = k/2 - k/4;
		hostPerPod = edgeSwitchPerPod * k/2;
	}
	...
	private void populateAdjacencyList(){
		...
		for (int i = 0; i < podNum; i++) {
			for (int j = 0; j < edgeSwitchPerPod; j++) {
				...
				ArrayList<Integer> remainingCore = new ArrayList<Integer> ();
				for (int t = 0; t < coreGroupSize; t++) {
					int tmpCore = j * coreGroupSize + t;
					if (!hostToCore.contains(tmpCore)) {
						remainingCore.add(tmpCore);
					}
				}
				...
				for (int t = 0; t < hostPerAggr; t++) {
					...
					int randIndex = (int)(Math.random()*remainingCore.size());
					coreSwitchIndex = remainingCore.get(randIndex);
					addBidirNeighbor(getNodeIndex("C"+coreSwitchIndex), getNodeIndex("E"+edgeSwitchIndex));
					...
					remainingCore.remove(randIndex);
				}
		...
		for (int i = 0; i < aggrSwitchNum; i++) {
			...
			weightEachNode[aggrIndex] = hostPerRealAggr;
			totalWeight += hostPerRealAggr;
		}
	}
	...
	public int svrToSwitch(int i)	//i is the server index. return the switch index.
	{
		...
		int pairIndex = hostInPod / (K/2);
		int hostInPair = hostInPod - pairIndex * (K/2);
		int index = podIndex * edgeSwitchPerPod + pairIndex;

		if (hostInPair < hostPerEdge) {
			//System.out.println(i + "--> E"+index);
			return getNodeIndex("E"+index);
		}
		else {
			//System.out.println(i + "--> A"+index);
			return getNodeIndex("A"+index);
		}
	}

	/*public static void main (String args[]) {
		FlatTreeCluster myTree = new FlatTreeCluster(6, 0);
